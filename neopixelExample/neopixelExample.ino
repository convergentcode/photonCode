...
void loop() {
  ...
  //function takes delay value, and then uses hardcoded colors 
  // set up to take an array to send two colors, or one color
  twoColor(20,  255, 0, 0, 0, 0, 255);

  //function takes a delay value, and then a value for r, g, b
  //strobeColor(50, 255, 0, 0);

  //colorAll(strip.Color(0, 255, 0), 50); // Cyan
}
...
// Slightly different, this makes the rainbow equally distributed throughout, then wait (ms)
void rainbowTwoFer(uint8_t wait) {
  uint16_t i, j;
...
// Slightly different, this makes the rainbow equally distributed throughout, then wait (ms)
void twoColor(uint8_t wait, uint8_t areone, uint8_t geeone, uint8_t beeone, uint8_t aretwo, uint8_t geetwo, uint8_t beetwo) {
  uint16_t i, j;
  
  for(j=0; j<256; j++) { // 1 cycle of all colors on wheel
    for(i=0; i< strip.numPixels(); i=i+2) {
      strip.setPixelColor(i, areone, geeone, beeone);
      strip.setPixelColor(i+1, aretwo, geetwo, beetwo);
    }
    strip.show();
    delay(wait);
  }
...
void strobeColor(uint8_t wait, uint8_t are, uint8_t gee, uint8_t bee) {
    uint16_t i, j, strobe;

  for(j=0; j<256; j++) { // 1 cycle of all colors on wheel
    for(i=0; i< strip.numPixels(); i++) {
      //strip.setPixelColor(i, Wheel(((i * 256 / strip.numPixels()) + j) & 255));
      strip.setPixelColor(i, are, gee, bee);
